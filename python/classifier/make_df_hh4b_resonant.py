...
def make_df(tree, fourTag=True, verbose=False):
    ...
    for iEvt in list(range(iEvtStart, iEvtEnd)):
        ...
        data["Jet0_pt"].append(copy(tree.jetPt[0]))
        data["Jet1_pt"].append(copy(tree.jetPt[1]))
        data["Jet2_pt"].append(copy(tree.jetPt[2]))
        data["Jet3_pt"].append(copy(tree.jetPt[3]))
        data["Jet0_eta"].append(copy(tree.jetEta[0]))
        data["Jet1_eta"].append(copy(tree.jetEta[1]))
        data["Jet2_eta"].append(copy(tree.jetEta[2]))
        data["Jet3_eta"].append(copy(tree.jetEta[3]))
        data["Jet0_phi"].append(copy(tree.jetPhi[0]))
        data["Jet1_phi"].append(copy(tree.jetPhi[1]))
        data["Jet2_phi"].append(copy(tree.jetPhi[2]))
        data["Jet3_phi"].append(copy(tree.jetPhi[3]))
        # jetMasses = []
        # for i in range(4):
        #     p_square = tree.jetPt[i] ** 2 * (np.cosh(tree.jetEta[i]) ** 2)
        ...
        for i in range(4):
            jets[i].SetPtEtaPhiM(tree.jetPt[i], tree.jetEta[i], tree.jetPhi[i], 0.0)
