...
class basicCNN(nn.Module):
    def __init__(
        ...
        super().__init__()
        self.name = "basicCNN_%d_%d_%d_%d_pdrop%.2f" % (
            dijetFeatures,
            quadjetFeatures,
            combinatoricFeatures,
            nodes,
        ...
        self.conv1 = nn.Sequential(
            *[nn.Conv1d(jetFeatures, dijetFeatures, 2, stride=2), nn.ReLU()]
        )
        self.conv2 = nn.Sequential(
            *[nn.Conv1d(dijetFeatures, quadjetFeatures, 2, stride=2), nn.ReLU()]
        )
        self.conv3 = nn.Sequential(
            *[nn.Conv1d(quadjetFeatures, combinatoricFeatures, 3, stride=1), nn.ReLU()]
        )

        self.line1 = nn.Sequential(*[nn.Linear(combinatoricFeatures, nodes), nn.ReLU()])
        self.line2 = nn.Sequential(
            *[nn.Linear(nodes, nodes), nn.ReLU(), nn.Dropout(p=pDropout)]
        )
...
class jetLSTM(nn.Module):
    ...
    def forward(self, js):  # j[event][jet][mu] l[event][nj]
        ...
        idx = (
            ls + torch.tensor(ls == 0, dtype=torch.long).to("cpu") - 1
...
class dijetReinforceLayer(nn.Module):
    def __init__(self, dijetFeatures, batchNorm=False, nAveraging=4):
        super().__init__()
...
class dijetResNetBlock(nn.Module):
    def __init__(
        ...
        self.reinforce1 = dijetReinforceLayer(
            self.nd, batchNorm=True, nAveraging=nAveraging
        )
        self.convJ = conv1d(
            self.nd,
            self.nd,
        ...
        self.reinforce2 = dijetReinforceLayer(
            self.nd, batchNorm=False, nAveraging=nAveraging
        )
...
class quadjetReinforceLayer(nn.Module):
    def __init__(self, quadjetFeatures, batchNorm=False, nAveraging=4):
        super().__init__()
        ...
        self.antisym = nn.Conv1d(
            self.nq, self.nq, 2, stride=2, bias=False, groups=self.nq
        )
...
class quadjetResNetBlock(nn.Module):
    def __init__(
        ...
        self.reinforce1 = quadjetReinforceLayer(
            self.nq, batchNorm=True, nAveraging=nAveraging
        )
        self.convD = conv1d(
            self.nq,
            self.nq,
        ...
        self.reinforce2 = quadjetReinforceLayer(
            self.nq, batchNorm=False, nAveraging=nAveraging
        )
...
class ResNet(nn.Module):
    def __init__(
        ...
        if self.useOthJets:
            self.othJetScaler = scaler(self.nj + 1, device)
        ...
        self.negativePhiCanJets = (
            torch.tensor([1, 1, -1, 1], dtype=torch.float).to(self.device).view(1, 4, 1)
        )
        self.negativeEtaCanJets = (
            torch.tensor([1, -1, 1, 1], dtype=torch.float).to(self.device).view(1, 4, 1)
        )
        self.negativePhiOthJets = (
            torch.tensor([1, 1, -1, 1, 1], dtype=torch.float)
            .to(self.device)
            .view(1, 5, 1)
        )
    ...
    def invPart(self, j, o, mask, d, q):
        ...
        d, d0 = self.dijetResNetBlock(
            j, d, j0=j0, d0=d0, o=o, mask=mask, debug=self.debug
        )
        ...
        q, q0 = self.quadjetResNetBlock(
            d, q, d0=d0, q0=q0, o=o, mask=mask, debug=self.debug
        )
    ...
    def forward(self, j, o, d, q):
        ...
        if self.useOthJets:
            ...
            oPt, oEta, oMass = (
                self.othJetPtGBN(oPt, mask),
                self.othJetEtaGBN(oEta, mask),
                self.othJetMassGBN(oMass, mask),
            )
        ...
        if self.training:
            randomR = torch.tensor(np.random.uniform(0, 2.0, 1), dtype=torch.float).to(
                self.device
            )

        # apply each of the symmetry transformations over which we will average
        # Rather than random rotation, rotate by -first jet's phi
        j = j.view(4, n, self.nj, 12)  ### TODO 12/9)
        jR, jRP, jRE, jRPE = j[0], j[1], j[2], j[3]

        if self.training:
            jR = self.rotate(jR, -jR[:, 2:3, 0])
        if self.useOthJets:
            o = o.view(4, n, 5, 12)
            oR, oRP, oRE, oRPE = o[0], o[1], o[2], o[3]
            if self.training:
                oR = self.rotate(oR, -oR[:, 2:3, 0])

        # flip phi
        if self.training:
            jRP = self.rotate(jRP, -jRP[:, 2:3, 0])
        jRP = self.flipPhi(jRP)
        if self.useOthJets:
            if self.training:
                oRP = self.rotate(oRP, -oRP[:, 2:3, 0])
            oRP = self.flipPhi(oRP, canJets=False)

        # flip eta
        if self.training:
            jRE = self.rotate(jRE, -jRE[:, 2:3, 0])
        jRE = self.flipEta(jRE)
        if self.useOthJets:
            if self.training:
                oRE = self.rotate(oRE, -oRE[:, 2:3, 0])
            oRE = self.flipEta(oRE, canJets=False)

        # flip phi and eta
        if self.training:
            jRPE = self.rotate(jRPE, -jRPE[:, 2:3, 0])
        jRPE = self.flipPhi(jRPE)
        jRPE = self.flipEta(jRPE)
        if self.useOthJets:
            if self.training:
                oRPE = self.rotate(oRPE, -oRPE[:, 2:3, 0])
