"""Freeze the paired fixed/composite affine null-calibration manifest."""
...
def main() -> None:
    ...
    if not source_audit.get("complete") or source_audit.get("completed") != 800:
        raise RuntimeError("No-correction source campaign is not complete")
    ...
    if pickle_path.exists():
        with pickle_path.open("rb") as handle:
            if pickle.load(handle) != rows:
                raise RuntimeError("Existing affine manifest differs from source")
    else:
    ...
    audit = {
        "campaign": "centered_poisson_affine_null_v1",
        "source_campaign": "centered_poisson_null_v1",
        "rows": len(rows),
        "cells": 8,
        "seeds_per_cell": 100,
