"""Same supplied test; only the long-double envelope stack is compiled.

Build: g++ -O2 -std=c++17 -fPIC -shared -fno-fast-math -ffp-contract=off
       affine_envelope_kernel.cpp -o affine_envelope_kernel.so
The unchanged Python reference is loaded privately, so importing this adapter
does not modify the reference module used for equivalence checks.
"""
import ctypes
...
name='_private_compiled_affine_ks_reference'
spec=importlib.util.spec_from_file_location(name,ROOT/'affine_weighted_ks_reference.py')
ref=importlib.util.module_from_spec(spec); sys.modules[name]=ref; spec.loader.exec_module(ref)
...
arrayLD=np.ctypeslib.ndpointer(dtype=np.longdouble,ndim=1,flags='C_CONTIGUOUS')
lib.affine_upper_stack.argtypes=[ctypes.c_size_t,array64,array64,arrayLD,arrayLD,arrayLD]
...
def upper_envelope(slopes,intercepts):
    ...
    s=slopes[order]; a=intercepts[order]
    first=np.r_[0,np.flatnonzero(s[1:]!=s[:-1])+1]
    a=np.maximum.reduceat(a,first); s=s[first]
    ...
    return ref._Envelope(left,right,hs[ids],ha[ids])

ref._upper_envelope=upper_envelope
affine_ks_test=ref.affine_ks_test
