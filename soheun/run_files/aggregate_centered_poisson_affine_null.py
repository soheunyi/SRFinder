"""Audit and aggregate the paired fixed/composite affine null campaign."""
...
def validate_affine(result: dict, *, hash_: str, mode: str) -> None:
    if (
        ...
        or result["multiplier_stream"]
        != "independent_class_seedsequence_spawn_v1"
    ):
        raise RuntimeError(f"{hash_}: invalid {mode} configuration")
    if not 0 <= result["fitted_t"] <= 1:
        raise RuntimeError(f"{hash_}: fitted t outside [0,1]")
    if not 0 <= result["maximizing_p_t"] <= 1:
        raise RuntimeError(f"{hash_}: maximizing-p t outside [0,1]")
    ...
    if mode == "fixed":
        ...
        exceedances = np.count_nonzero(
            values + result["numerical_tolerance"] >= result["statistic"]
        )
        if exceedances != result["max_exceedances"]:
...
def main() -> None:
    ...
    for path in paths:
        ...
        if (
            ...
            raise RuntimeError(f"{hash_}: campaign configuration mismatch")
        validate_affine(result["fixed"], hash_=hash_, mode="fixed")
        validate_affine(
            result["composite"],
            hash_=hash_,
            mode="composite_supremum",
        )
        none = result["no_correction"]
        fixed = result["fixed"]
        ...
        rows.append(
            {
                **target,
                "none_p_value": none["p_value"],
                "none_reject": none["reject"],
                "fixed_p_value": fixed["p_value"],
                "fixed_reject": fixed["reject"],
                "composite_p_value": composite["p_value"],
                "composite_reject": composite["reject"],
                "fixed_t": fixed["fitted_t"],
                "identity_t": fixed["identity_t"],
                "fixed_t_minus_identity": fixed["fitted_t"] - fixed["identity_t"],
                "correction_value_L": fixed["correction_value_L"],
                "correction_value_U": fixed["correction_value_U"],
                "correction_endpoint_ratio": correction_ratio,
                "fixed_fit_objective": fixed["fit_objective"],
                "fixed_statistic": fixed["statistic"],
                "composite_statistic": composite["statistic"],
    ...
    summary = grouped.agg(
        ...
        fixed_rejections=("fixed_reject", "sum"),
        fixed_rejection_rate=("fixed_reject", "mean"),
        composite_rejections=("composite_reject", "sum"),
        composite_rejection_rate=("composite_reject", "mean"),
        mean_none_p_value=("none_p_value", "mean"),
        mean_fixed_p_value=("fixed_p_value", "mean"),
        mean_composite_p_value=("composite_p_value", "mean"),
        ...
        mean_correction_endpoint_ratio=("correction_endpoint_ratio", "mean"),
        ...
        composite_p_lower_boundary_rate=("composite_maximizing_p_t", lambda x: np.mean(x <= 1e-12)),
        composite_p_upper_boundary_rate=("composite_maximizing_p_t", lambda x: np.mean(x >= 1 - 1e-12)),
    ...
    for prefix in ("none", "fixed", "composite"):
        intervals = [
    ...
    detailed.to_csv(OUT / "centered_poisson_affine_null_detailed.csv", index=False)
    summary.to_csv(OUT / "centered_poisson_affine_null_summary.csv", index=False)
    audit = {
        "complete": True,
