...
class StackedAttentionClassifier(pl.LightningModule):
    """
    Stacked Attention classifier. This implements a stacked version of the AttentionClassifier,
    which is the decoder architecture used in FvT classifier.
    ...
    Args:
        num_stacks: number of independent AttentionClassifiers in the stack.
        num_classes: number of output classes for each classifier.
        dim_quadjet_features: quadjet feature dimension (input to AttentionClassifier).
        run_names: list of run names for each stack.
        stacked_run_name: name for the overall stacked model.
        device: device to run the model on.
        depth: depth configuration for the ResNet blocks in AttentionClassifier.
    """
    ...
    def __init__(
        ...
        # Stacking multiple AttentionClassifier modules
        self.attention_classifiers = nn.ModuleList(
            [
                AttentionClassifier(
                    dim_q=self.dim_q,
    ...
    def forward(self, q: torch.Tensor) -> torch.Tensor:
        """
        Performs forward pass through all stacked attention classifiers.

        Args:
            q: float tensor of shape (batch_size, num_stacks, dim_quadjet_features, 3)
        Returns:
    ...
    def predict(self, q):
        ...
        q = q.to(self.device)  # Optional explicit transfer
        logits_stack = self(q)
        return logits_stack

    @torch.no_grad()
    def predict_proba(self, q):
        """Returns probability predictions for all stacks."""
        q = q.to(self.device)
        logits_stack = self(q)
    ...
    def configure_optimizers(self):
        ...
        for i, attention_clf in enumerate(self.attention_classifiers):
            # You can customize optimizer settings per attention_clf if needed
    ...
    def fit(
        ...
        if self.early_stop_patience is not None:
            raise NotImplementedError(
                "Early stopping not implemented for stacked AttentionClassifier"
            )
